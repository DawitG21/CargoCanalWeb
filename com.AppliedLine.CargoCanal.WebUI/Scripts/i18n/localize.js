        ...
        .factory('localize', ['$http', '$rootScope', '$window', '$filter', function ($http, $rootScope, $window, $filter) {
            var localize = {
                ...
                buildUrl: function () {
                    ...
                    return 'src/assets/i18n/resources-locale_' + localize.language + '.json';
                },
                ...
                getLocalizedString: function (value) {
                    ...
                    if ((localize.dictionary !== []) && (localize.dictionary.data) && (localize.dictionary.data.length > 0)) {
                        // use the filter service to only return those entries which match the value
                        // and only take the first result
                        var entry = $filter('filter')(localize.dictionary.data, function (element) {
                            return element.key === value;
                        }
                        )[0];

                        // set the result
                        if (entry != undefined) result = entry.value;
                    }
